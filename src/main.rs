...
fn grab_interface_profiles(
    ...
    let mut wlan_profile_ptr = std::ptr::null_mut();

    let result = unsafe { WlanGetProfileList(handle, interface_guid, None, &mut wlan_profile_ptr) };
...
fn traverse_xml_tree(xml: &XmlElement, node_path: &[&str]) -> Option<String> {
    ...
    let last_node_name = *node_path.last()?; // Desreferencia aquí

    'node_traverse: for node in node_path {
        let node_name = OsString::from_wide(&node.encode_utf16().collect::<Vec<u16>>());

        for i in 0..subtree_list.Length().ok()? {
            let subtree_value = subtree_list.Item(i).ok()?;
            let element_name = subtree_value.NodeName().ok()?;
...
fn main() {
    let wlan_handle = open_wlan_handle(WLAN_API_VERSION_2_0)
        .expect("Failed to open WLAN handle!");
    ...
    for interface_info in interface_list {
        ...
        for profile in wlan_profile_list {
            ...
            let auth_type = traverse_xml_tree(
                &root,
                &["MSM", "security", "authEncryption", "authentication"],
            )
            .unwrap_or_else(|| "UNKNOWN".to_string());

            // Try to grab the keyMaterial regardless of auth type
                ...
                traverse_xml_tree(&root, &["MSM", "security", "sharedKey", "keyMaterial"])
            {
                ...
                println!(
                    "Wi-Fi Name: {}, Authentication: {}, Password: <NOT FOUND>",
                    profile_name.to_string_lossy(),
                    auth_type
                );
            }
